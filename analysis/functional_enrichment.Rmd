...
metanalysis <- read_csv("data/meta-analise.csv") %>%
  janitor::clean_names() %>%
...
write_csv(enrich_terms, "results/metanalysis_go_enrichment.csv")
```
...
ggplot2::ggsave("results/go_barplot.pdf")
```
...
ggplot2::ggsave("results/go_dotplot.pdf", height = 10)
```
...
write_csv(enrich_terms, "results/metanalysis_kegg_enrichment.csv")
```
...
ggplot2::ggsave("results/kegg_barplot.pdf")
```

```{r kegg-plot-2, fig.height=7}
dotplot(enrich_kegg, showCategory = 10, font.size = 12)

ggplot2::ggsave("results/kegg_dotplot.pdf", height = 7)
```
